...
[
  ...
  'isMatch', 'isError', 'each', 'omit', 'omitBy', 'compact', 'clone',
].forEach((module) => {
...
const webScraper = {
  ...
  parseData(inputData, contextElement) {
    ...
    if (_.isString(data)) {
      return this.useElement(contextElement, data)
    ...
    if (data._elem || data._value || data._follow || data._filter) {
      return this.useElement(contextElement, data)
        .then((elem) => this.getObjectValue(data, elem));
    }
    ...
    return Promise.all(keys.map((key) => {
      ...
      if (_.isObject(item)) {
        return this.getElement(item, contextElement, null)
          .then((elem) => this.parseData(item, elem))
          .then((val) => {
            json[key] = val;
          })
          .catch(this.logger.setProperty(json, key, {}));
      ...
      return this.getElement(item, contextElement)
        .then((elem) => {
  ...
  useElement(contextElement, data) {
    if (contextElement) {
  ...
  getObjectValue(data, contextElement) {
    ...
    if (!hasKeys && follow) {
      ...
      return this.follow(follow, ctxElement);
    }

    return this.parseData(filteredData, ctxElement)
      .then((parsedData) => {
        ...
        return this.follow(follow, ctxElement, parsedData);
      });
  },
  ...
  follow(follow, contextElement, json = {}) {
    ...
    return this.getElement(followObj, contextElement, contextElement)
