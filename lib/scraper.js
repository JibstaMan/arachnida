...
const webScraper = {
  ...
  parseData(data, contextElement) {
    ...
    if (_.isArray(data)) {
      return this.getArrayValue(data[0], contextElement);
    }
  ...
  getArrayValue(data, contextElement) {
    return this.getElementList(data, contextElement)
      .then((elements) => {
  ...
  getElementList(data, contextElement) {
    return this.getElement(data, contextElement)
  ...
  getElement(data, contextElement) {
    ...
    if (_.isObject(data) && data._remove) {
      $elem.find(data._remove).remove();
    }
