...
Scraper.prototype.getArrayValue = function(item, cb)
{
    let elements = this.getElement(item).toArray();
...
Scraper.prototype.getObjectValue = function(object, elem, cb)
{
    if (!object._elem && !elem)
    {
        // no selector, so this is a nested object.
        return this.parseData(object, cb);
    }

    // we'll be modifying the object, so clone it first.
    let item = _.cloneDeep(object);
    ...
    if (!hasKeys && follow)
    {
        ...
        // an obect with only a _follow property.
